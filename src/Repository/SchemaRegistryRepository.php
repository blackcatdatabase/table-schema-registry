...
namespace BlackCat\Database\Packages\SchemaRegistry\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SchemaRegistry\Definitions;
use BlackCat\Database\Packages\SchemaRegistry\Criteria;
use BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto as Dto;
use BlackCat\Database\Packages\SchemaRegistry\Mapper\SchemaRegistryDtoMapper as RowMapper;
...
final class SchemaRegistryRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SchemaRegistry\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'checksum', 'applied_at', 'meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'system_name', 'component', 'version' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SchemaRegistry\Definitions::table());
            return $bulk->upsertMany($rows, [ 'system_name', 'component', 'version' ], [ 'checksum', 'applied_at', 'meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto|null */
    public function getBySystemNameAndComponentAndVersion(string $systemName, string $component, string $version, bool $asDto = false): array|\BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto|null {
        $row = $this->getByUnique([ 'system_name' => $systemName, 'component' => $component, 'version' => $version ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SchemaRegistry\Mapper\SchemaRegistryDtoMapper::fromRow($row);
    }
    public function existsBySystemNameAndComponentAndVersion(string $systemName, string $component, string $version): bool {
        $where = 't.' . Ident::q($this->db, 'system_name') . ' = :uniq_system_name' . ' AND ' . 't.' . Ident::q($this->db, 'component') . ' = :uniq_component' . ' AND ' . 't.' . Ident::q($this->db, 'version') . ' = :uniq_version';
        return $this->exists($where, [ 'uniq_system_name' => $systemName, 'uniq_component' => $component, 'uniq_version' => $version ]);
    }
    /** @return int|string|null */
    public function getIdBySystemNameAndComponentAndVersion(string $systemName, string $component, string $version) {
        $row = $this->getBySystemNameAndComponentAndVersion($systemName, $component, $version);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto|null {
