...
use BlackCat\Database\Packages\SchemaRegistry\Repository\SchemaRegistryRepositoryInterface;
...
class SchemaRegistryRepository implements SchemaRegistryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'system_name', 'component', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\SchemaRegistry\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'checksum', 'applied_at', 'meta' ]);
          return count($rows);
        }
    ...
    public function getBySystemNameAndComponentAndVersion(string $systemName, string $component, string $version, bool $asDto = false): array|\BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto|null {
        return $this->getByUnique([ 'system_name' => $systemName, 'component' => $component, 'version' => $version ], $asDto);
    }
    ...
    public function getIdBySystemNameAndComponentAndVersion(string $systemName, string $component, string $version) {
        $row = $this->getBySystemNameAndComponentAndVersion($systemName, $component, $version, false);
