...
namespace BlackCat\Database\Packages\SchemaRegistry\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SchemaRegistry\Dto\SchemaRegistryDto;
use BlackCat\Database\Packages\SchemaRegistry\Mapper\SchemaRegistryDtoMapper;
 ...
 */
final class SchemaRegistryAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SchemaRegistryRepository $schemaRegistryRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'schema_registry' => $this->schemaRegistryRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SchemaRegistry\ModuleException("Record {$idStr} not found for locking.");
            }
